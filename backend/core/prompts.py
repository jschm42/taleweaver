...
GM_MECHANICS_SUFFIX = (
    ...
    "You can also toggle `is_attackable: false` for NPCs that become friendly or essential, or `true` if they become hostile targets. "
...
GM_STORY_MECHANICS_SUFFIX = (
    ...
    "- If an NPC moves to a different scene, use `moved_entities` with `to_scene_id`.\n"
...
GM_CHAT_MINIMAL_RULE_PASS_PROMPT = (
    ...
    "If an NPC clearly moves to another room or changes their physical spot, reflect this in `moved_entities` (fields: `entity_id`, `to_scene_id`) or `updated_entities` (fields: `entity_id`, `spatial_position`). "
    ...
    "Return only these intent fields when justified: `new_inventory_items`, `removed_inventory_item_ids`, `updated_inventory_items`, `spawned_items`, `hp_change`, `stamina_change`, `mana_change`, `completed_quest_ids`, `earned_award_keys`, `remember_notes`, `forget_notes`, `clear_notes`, `game_completed`, `game_over`, `status_note`, `moved_entities`, `updated_entities`, `new_scene_id`, `exit_label`, `extra_time_minutes`.\n\n"
    "OPEN QUESTS (REDUCED):\n"
    "{quests_json}\n"
