...
GM_MECHANICS_SUFFIX = (
    ...
    "For locked CONTAINER objects, evaluate their unlock condition in the story context; when fulfilled, set `updated_entities.locked=false` for that container entity so the player can open it. "
...
GM_STORY_MECHANICS_SUFFIX = (
    ...
    "UNRESOLVED QUESTS:\n"
    "{quests_json}\n"
    "For locked CONTAINER objects, evaluate their unlock condition in context; when fulfilled, set `updated_entities.locked=false` for that container entity.\n"
    "Evaluate if any of the following Awards have been earned based on the current action:\n"
    "{awards_json}\n"
    "If an award is earned, return its key in 'earned_award_keys'. Only grant an award once per session. You CANNOT create new awards; only use the keys provided in the list below.\n"
    "NOTES TOOL:\n"
